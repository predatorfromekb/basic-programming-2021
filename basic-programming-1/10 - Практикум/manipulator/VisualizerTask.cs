...
namespace Manipulation
{
    public static class VisualizerTask
    {
        ...
        {
            if (key.KeyCode == Keys.Q) Shoulder += 0.1;
            if (key.KeyCode == Keys.A) Shoulder -= 0.1;
            if (key.KeyCode == Keys.W) Elbow += 0.1;
            if (key.KeyCode == Keys.S) Elbow -= 0.1;
            // Shoulder + Elbow + Wrist + Alpha = 0
        ...
        {
            var jointAngles = ManipulatorTask.MoveManipulatorTo(X, Y, Alpha);
            if (!jointAngles.Any(double.IsNaN))
            {
                Shoulder = jointAngles[0];
                Elbow = jointAngles[1];
                Wrist = jointAngles[2];
            }
        }
        ...
        {
            var pos = ConvertWindowToMath(new PointF(e.X, e.Y), GetShoulderPos(form));
            X = pos.X;
            Y = pos.Y;
            UpdateManipulator();
            form.Invalidate();
        ...
        public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
        {
            ...
            var font = new Font(SystemFonts.DefaultFont.FontFamily, 12);
            graphics.DrawString($"X={X:0}, Y={Y:0}, Alpha={Alpha:0.00}", font, Brushes.DarkRed, 10, 10);
            DrawReachableZone(graphics, ReachableAreaBrush, UnreachableAreaBrush, shoulderPos, joints);

            var prev = new PointF(0, 0);
            var r = 10f;
            foreach (var jointPos in joints)
            {
                var windowPrevJointPos = ConvertMathToWindow(prev, shoulderPos);
                var windowCurrentJointPos = ConvertMathToWindow(jointPos, shoulderPos);

                graphics.DrawLine(
                    ManipulatorPen,
                    windowPrevJointPos,
                    windowCurrentJointPos);
                graphics.FillEllipse(
                    JointBrush,
                    windowPrevJointPos.X - r, windowPrevJointPos.Y - r,
                    2 * r, 2 * r);
                prev = jointPos;
            }
        }
